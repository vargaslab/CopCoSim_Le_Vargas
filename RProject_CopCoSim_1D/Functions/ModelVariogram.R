ModelVariogram<-function (CoorX, CoorY, P1, P2, NInt, lags, Direccion, Tol, Modelo, 
          Sill1, Sill2, Sill3, Nugget1, Nugget2, Nugget3, Alcance, Xlabel = "Distance", 
          NomP1, NomP2, NomP1P2) 
{
  ...
  plot(v.cross$dist[(k0 + k1 + 1):(k0 + k1 + k2)], v.cross$gamma[(k0 + 
                                                                    k1 + 1):(k0 + k1 + k2)], xlab = Xlabel, ylab = expression(bold("Semivariance")), 
  ...
  legend( "topright", c("Sill+Nugget", "Range"), col = c("Blue", "green"), lty = c(3, 
                                                                                           5), bty = "n")
  # 0.7 * MaxX1, 0.5 * MaxY1 ,  "Variance") , "Red"), , 1)
  title(NomP1, cex.main = 1.5)
  par(mar = c(5, 4, 2, 1))
  plot(v.cross$dist[1:k0], v.cross$gamma[1:k0], xlab = Xlabel, 
       ylab = expression(bold("Semivariance")), xlim = c(0, MaxX2), ylim = c(0, 
  ...
  legend("topright", c("Sill+Nugget", "Range"), col = c("Blue", "green"), lty = c(3, 
                                                                                           5), bty = "n")
  #0.7 * MaxX3, 0.5 * MaxY3,, "Variance") , "Red")  , 1)
